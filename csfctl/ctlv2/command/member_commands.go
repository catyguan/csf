...
func NewMemberCommand() cli.Command {
	return cli.Command{
		...
		Subcommands: []cli.Command{
			...
			{
				Name:      "add",
				Usage:     "add a new member to the csf cluster",
				ArgsUsage: "<name> <peerURL>",
				Action:    actionMemberAdd,
			},
			{
				Name:      "remove",
				Usage:     "remove an existing member from the csf cluster",
				ArgsUsage: "<memberID>",
				Action:    actionMemberRemove,
			},
			{
				Name:      "update",
				Usage:     "update an existing member in the csf cluster",
...
func actionMemberAdd(c *cli.Context) error {
	...
	fmt.Printf("initial-cluster-state: existing\n")
	return nil
}
