...
func NewBackupCommand() cli.Command {
	return cli.Command{
		Name:      "backup",
		Usage:     "backup an csf directory",
		ArgsUsage: " ",
		Flags: []cli.Flag{
			cli.StringFlag{Name: "data-dir", Value: "", Usage: "Path to the csf data dir"},
			cli.StringFlag{Name: "wal-dir", Value: "", Usage: "Path to the csf wal dir"},
...
func handleBackup(c *cli.Context) error {
	...
	case wal.ErrSnapshotNotFound:
		...
		fmt.Printf("csfctl will add it back. Start auto fixing...")
	default:
		log.Fatal(err)
	}
	var metadata csfserverpb.Metadata
