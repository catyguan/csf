...
func SRVGetCluster(name, dns string, defaultToken string, apurls types.URLs) (string, string, error) {
	...
	failCount := 0
	err := updateNodeMap("csf-server-ssl", "https://")
	srvErr := make([]string, 2)
	if err != nil {
		srvErr[0] = fmt.Sprintf("error querying DNS SRV records for _csf-server-ssl %s", err)
		failCount++
	}
	err = updateNodeMap("csf-server", "http://")
	if err != nil {
		srvErr[1] = fmt.Sprintf("error querying DNS SRV records for _csf-server %s", err)
		failCount++
	}
	if failCount == 2 {
