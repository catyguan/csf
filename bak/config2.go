...
import (
	...
	"github.com/catyguan/csf/pkg/flags"
...
func NewAppConfig(exeName string) *AppConfig {
	cfg := &AppConfig{
		Config: *NewConfig(),
	...
	cfg.configFlags = configFlags{
		FlagSet: flag.NewFlagSet("csf", flag.ContinueOnError),
	...
	fs := cfg.FlagSet
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, fmt.Sprintf(usageline, exeName))
	}
	...
	fs.StringVar(&cfg.InitialClusterToken, "initial-cluster-token", cfg.InitialClusterToken, "Initial cluster token for the CSF cluster during bootstrap.")
	...
	// logging
	fs.BoolVar(&cfg.Debug, "debug", false, "Enable debug-level logging for CSF.")
	fs.StringVar(&cfg.LogPkgLevels, "log-package-levels", "", "Specify a particular log level for each csf package (eg: 'csfmain=CRITICAL,csfserver=DEBUG').")
...
func (cfg *AppConfig) Parse(arguments []string) error {
	...
	if cfg.configFile != "" {
		...
		err = cfg.AppConfigFromFile(cfg.configFile)
	} else {
		err = cfg.AppConfigFromCmdLine()
	}
	return err
}

func (cfg *AppConfig) AppConfigFromCmdLine() error {
	err := flags.SetFlagsFromEnv("CSF", cfg.FlagSet)
	if err != nil {
		plog.Fatalf("%v", err)
	}
...
func (cfg *AppConfig) AppConfigFromFile(path string) error {
	eCfg, err := ConfigFromFile(path)
	if err != nil {
		return err
	}
	cfg.Config = *eCfg
...
func (cfg *AppConfig) mayBeProxy() bool {
...
func (cfg *AppConfig) Validate() error {
	...
	if err == ErrUnsetAdvertiseClientURLsFlag && cfg.mayBeProxy() {
		return nil
	}
	return err
}

func (cfg AppConfig) isProxy() bool               { return cfg.proxy.String() != proxyFlagOff }
func (cfg AppConfig) isReadonlyProxy() bool       { return cfg.proxy.String() == proxyFlagReadonly }
func (cfg AppConfig) shouldFallbackToProxy() bool { return cfg.fallback.String() == fallbackFlagProxy }
