...
import (
	...
	"github.com/catyguan/csf/cluster/stats"
	...
	"github.com/catyguan/csf_dev/csfserver"
	"github.com/catyguan/csf_dev/csfserver/api"
	"github.com/catyguan/csf_dev/csfserver/api/v2http/httptypes"
	"github.com/catyguan/csf_dev/csfserver/auth"
	"github.com/catyguan/csf_dev/csfserver/csfserverpb"
...
func NewClientHandler(server *cluster.CSFNode, timeout time.Duration) http.Handler {
	sec := server.Cfg.ACL
...
func (h *membersHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if !HasCSFAccess(h.sec, r, h.clientCertAuthEnabled) {
		WriteNoAuth(w, r)
		return
	}
	...
	case "GET":
		...
		case "leader":
			id := h.server.Leader()
			if id == 0 {
				writeError(w, r, NewHTTPError(http.StatusServiceUnavailable, "During election"))
				return
			}
		...
		default:
			writeError(w, r, NewHTTPError(http.StatusNotFound, "Not found"))
		}
	}
}
...
func newMemberCollection(ms []*membership.Member) *MemberCollection {
	c := MemberCollection(make([]Member, len(ms)))
...
func newMember(m *membership.Member) Member {
	tm := Member{
		ID:         m.ID.String(),
		Name:       m.Name,
