...
var (
	plog = capnslog.NewPackageLogger("github.com/catyguan/csf", "csfserver/membership")
)
...
func NewMember(clusterName string, id types.ID, name string, peerURLs, clientURLs types.URLs) *Member {
	m := &Member{
		ID:         id,
		Name:       name,
		PeerURLs:   peerURLs,
...
func (m *Member) PickPeerURL() url.URL {
...
func (m *Member) Clone() *Member {
	...
	if m.PeerURLs != nil {
		mm.PeerURLs = make(types.URLs, len(m.PeerURLs))
		copy(mm.PeerURLs, m.PeerURLs)
	}
	if m.ClientURLs != nil {
		mm.ClientURLs = make(types.URLs, len(m.ClientURLs))
