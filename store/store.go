...
func (s *store) Set(nodePath string, dir bool, value string, expireOpts TTLOptionSet) (*Event, error) {
	...
	if getErr != nil && getErr.ErrorCode != csfErr.EcodeKeyNotFound {
		err = getErr
		return nil, err
	}
...
func (s *store) CompareAndSwap(nodePath string, prevValue string, prevIndex uint64,
	...
	if s.readonlySet.Contains(nodePath) {
		return nil, csfErr.NewError(csfErr.EcodeRootROnly, "/", s.CurrentIndex)
	}
	...
	if n.IsDir() { // can only compare and swap file
		err = csfErr.NewError(csfErr.EcodeNotFile, nodePath, s.CurrentIndex)
		return nil, err
	}
	...
	if ok, which := n.Compare(prevValue, prevIndex); !ok {
		...
		err = csfErr.NewError(csfErr.EcodeTestFailed, cause, s.CurrentIndex)
		return nil, err
	}
...
func (s *store) CompareAndDelete(nodePath string, prevValue string, prevIndex uint64) (*Event, error) {
	...
	if n.IsDir() { // can only compare and delete file
		return nil, csfErr.NewError(csfErr.EcodeNotFile, nodePath, s.CurrentIndex)
	}
	...
	if ok, which := n.Compare(prevValue, prevIndex); !ok {
		cause := getCompareFailCause(n, which, prevValue, prevIndex)
		return nil, csfErr.NewError(csfErr.EcodeTestFailed, cause, s.CurrentIndex)
	}
...
func (s *store) walk(nodePath string, walkFunc func(prev *node, component string) (*node, *csfErr.Error)) (*node, *csfErr.Error) {
...
func (s *store) Update(nodePath string, newValue string, expireOpts TTLOptionSet) (*Event, error) {
	...
	if n.IsDir() && len(newValue) != 0 {
		...
		return nil, csfErr.NewError(csfErr.EcodeNotFile, nodePath, currIndex)
	}
...
func (s *store) internalCreate(nodePath string, dir bool, value string, unique, replace bool,
	expireTime time.Time, action string) (*Event, *csfErr.Error) {
	...
	// we do not allow the user to change "/"
	if s.readonlySet.Contains(nodePath) {
		return nil, csfErr.NewError(csfErr.EcodeRootROnly, "/", currIndex)
	}
	...
	if n != nil {
		if replace {
			...
			return nil, csfErr.NewError(csfErr.EcodeNodeExist, nodePath, currIndex)
		}
	}
...
func (s *store) internalGet(nodePath string) (*node, *csfErr.Error) {
	nodePath = path.Clean(path.Join("/", nodePath))

	walkFunc := func(parent *node, name string) (*node, *csfErr.Error) {

		if !parent.IsDir() {
			err := csfErr.NewError(csfErr.EcodeNotDir, parent.Path, s.CurrentIndex)
			return nil, err
		}
		...
		return nil, csfErr.NewError(csfErr.EcodeKeyNotFound, path.Join(parent.Path, name), s.CurrentIndex)
	}
...
func (s *store) checkDir(parent *node, dirName string) (*node, *csfErr.Error) {
	...
	if ok {
		...
		return nil, csfErr.NewError(csfErr.EcodeNotDir, node.Path, s.CurrentIndex)
	}
