...
import (
	...
	"github.com/catyguan/csf/csfserver/api/v2http"
...
var plog = capnslog.NewPackageLogger("github.com/catyguan/csf", "csfembed")

// Cluster contains a running etcd server and its listeners.
type Cluster struct {
	Peers   []net.Listener
	Clients []net.Listener
	Server  *csfserver.CsfServer
...
// StartCluster launches the etcd server and HTTP handlers for client/server communication.
// The returned Cluster.Server is not guaranteed to have joined the cluster. Wait
// on the Cluster.Server.ReadyNotify() channel to know when it completes and is ready for use.
func StartCluster(inCfg *Config) (e *Cluster, err error) {
	...
	if !isMemberInitialized(cfg) {
		urlsmap, token, err = cfg.PeerURLsMapAndToken("csf")
		if err != nil {
			return e, fmt.Errorf("error setting up initial cluster: %v", err)
		}
	}

	srvcfg := &csfserver.ServerConfig{
		Name:                    cfg.Name,
	...
	if e.Server, err = csfserver.NewServer(srvcfg); err != nil {
		return
	}
...
func (e *Cluster) Config() Config {
	return e.cfg
}

func (e *Cluster) Close() {
	for _, sctx := range e.sctxs {
		sctx.cancel()
	}
	for i := range e.Peers {
...
func (e *Cluster) Err() <-chan error { return e.errc }
...
func (e *Cluster) OnReady(waitDuration time.Duration) bool {
	select {
...
func (e *Cluster) serve() (err error) {
