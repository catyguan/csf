...
import (
	...
	pb "github.com/catyguan/csf/auth/authpb"
...
func (as *authStore) RoleGrantPermission(r *pb.AuthRoleGrantPermissionRequest) (*pb.AuthRoleGrantPermissionResponse, error) {
	...
	plog.Noticef("role %s's permission of key %s is updated as %s", r.Name, r.Perm.Key, pb.Permission_Type_name[int32(r.Perm.PermType)])

	return &pb.AuthRoleGrantPermissionResponse{}, nil
}

func (as *authStore) isOpPermitted(userName string, revision uint64, key, rangeEnd []byte, permTyp pb.Permission_Type) error {
...
func (as *authStore) IsPutPermitted(authInfo *AuthInfo, key []byte) error {
	return as.isOpPermitted(authInfo.Username, authInfo.Revision, key, nil, pb.WRITE)
}

func (as *authStore) IsRangePermitted(authInfo *AuthInfo, key, rangeEnd []byte) error {
	return as.isOpPermitted(authInfo.Username, authInfo.Revision, key, rangeEnd, pb.READ)
}

func (as *authStore) IsDeleteRangePermitted(authInfo *AuthInfo, key, rangeEnd []byte) error {
	return as.isOpPermitted(authInfo.Username, authInfo.Revision, key, rangeEnd, pb.WRITE)
}
...
func getUser(tx backend.BatchTx, username string) *pb.User {
...
func getAllUsers(tx backend.BatchTx) []*pb.User {
...
func putUser(tx backend.BatchTx, user *pb.User) {
	b, err := user.Marshal()
	if err != nil {
...
func getRole(tx backend.BatchTx, rolename string) *pb.Role {
...
func getAllRoles(tx backend.BatchTx) []*pb.Role {
...
func putRole(tx backend.BatchTx, role *pb.Role) {
	b, err := role.Marshal()
	if err != nil {
...
func hasRootRole(u *pb.User) bool {
